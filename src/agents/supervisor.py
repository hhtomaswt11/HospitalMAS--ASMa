...
class Supervisor(Agent):
    ...
    class PeriodicDumperBehaviour(PeriodicBehaviour):
        async def run(self):
            ...
            # Persiste as métricas de simulação (apenas o supervisor H1 escreve o
            # ficheiro global para evitar race; o H2 escreve o seu ficheiro próprio).
            if self.agent._hospital_id == 1:
                metrics.dump_metrics("data/metrics.json")
            metrics.dump_metrics(f"data/metrics_h{self.agent._hospital_id}.json")
    ...
    class MonitorBehaviour(CyclicBehaviour):
        async def run(self):
            try:
                ...
                if performative == "inform" and msg_type == "resource_status":
                ...
                elif performative == "inform" and msg_type == "patient_attended":
                    # Notificação de um coordenador: doente foi atendido.
                    data = json.loads(msg.body)
                    doente_jid = data.get("doente_jid")
                    tipo = data.get("tipo", "urgencia")
                    attended_at = data.get("attended_at")
                    if doente_jid:
                        metrics.register_attended(
                            doente_jid, tipo,
                            hospital_id=self.agent._hospital_id,
                            attended_at=attended_at,
                        )
                    log(self.agent._supervisor_name,
                        f"[MÉTRICAS] Atendimento registado: {data.get('nome', doente_jid)} "
                        f"(tipo={tipo})", "CYAN")

                elif performative == "inform" and msg_type == "patient_abandoned":
                    # Notificação de um coordenador: doente esgotou retentativas.
                    data = json.loads(msg.body)
                    doente_jid = data.get("doente_jid")
                    tipo = data.get("tipo", "urgencia")
                    if doente_jid:
                    ...
                    log(self.agent._supervisor_name,
                        f"[MÉTRICAS] Abandono registado: {data.get('nome', doente_jid)} "
                        f"(tipo={tipo}, motivo={data.get('motivo', '?')})", "RED")
